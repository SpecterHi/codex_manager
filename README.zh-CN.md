...
`codex_manager` 是一个面向 Codex session 的本地优先监督台。

它解决的不是“agent 能不能干活”，而是：

> “我已经有很多真实的 Codex 会话在跑了，怎么才能稳定地观察、判断、接管、续跑，而不是被一堆客户端和超大的 session 文件拖垮？”
...
- **多会话监督**
  - 同时扫很多个 Codex session，而不是只盯着一个当前对话
- **有界的实时观察**
  - 看最近事件窗口、commentary、工具调用、工具输出摘要、完成标记
  - 默认不去加载整份超大的 `.jsonl`
- **适合手机介入**
...
- **保守的“持续推进”**
  ...
  - 不靠“空闲了几分钟”这种猜测
  - 有单实例 supervisor 锁，避免双重巡检
- **小规模多机控制**
  - 一个控制面切换几台 Codex 主机
  - 通过 `ssh + sudo -n` 自举远端 helper，不用手工长期维护第二份副本
- **更安全的远端接管**
  - 接管前先检查远端 service / API / 版本是否兼容
...
> “我已经有一批真实 session 在本机、WSL 或远端机器上运行，我需要一个低内存、低意外、可观察、可插话的值班台。”

## 和同类项目相比，强在哪里

| 工具 | 更擅长什么 | `codex_manager` 的差异 |
| --- | --- | --- |
| [OpenAI Codex app](https://openai.com/index/introducing-the-codex-app/) | 单线程主工作流、官方多 agent 体验、diff/worktree 代码工作流 | `codex_manager` 更强在“很多既有 session 的监督与值班” |
| [Happy](https://github.com/slopus/happy) | 手机/Web 远控、推送提醒、跨设备切换、加密远控体验 | `codex_manager` 不要求你用 wrapper 启动 agent，更适合观察已经存在的会话资产 |
| [cc-connect](https://github.com/chenhg5/cc-connect) | 把本地 agent 接入聊天平台，做 ChatOps | `codex_manager` 是浏览器控制面，重点是可观察性和介入，不是聊天入口 |

这里真正的差异点是：

- **避免默认全量吃内存**
  - 核心 UI 围绕 recent tail / bounded live window，而不是整份 transcript hydration
- **保守的 supervision 逻辑**
...
- 控制面机器负责出 UI
- 每台目标机器各自跑一个 loopback 绑定的 `codex_manager`
...
- `codex_sessions.py`
  - CLI 入口
- `codex_sessions_web.py`
  - Web UI、API、远端代理、监督逻辑
- `codex_sessions_bootstrap.py`
  - `ssh + sudo` 远端自举 / 升级 helper
